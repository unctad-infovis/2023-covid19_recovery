...
function ColumnChart({
  ...
  const dummy_data = {
    data: [{ name: '', y: null }, { name: '2020', y: null }, { name: '2021', y: null }, { name: '2022', y: null }],
    name: 'Dummy data',
    showInLegend: false,
    xAxis: 1
  };
    ...
    Highcharts.chart(`chartIdx${idx}`, {
      ...
      chart: {
        events: {
          load() {
            ...
            this.renderer.image(`${window.location.href.includes('unctad.org') ? 'https://storage.unctad.org/2023-covid19_recovery/' : './'}assets/img/unctad_logo.svg`, 5, 15, 80, 100).add();
          }
        },
        height: 650,
        resetZoomButton: {
        ...
        chartOptions: {
          chart: {
            events: {
              load() {
                // eslint-disable-next-line react/no-this-in-sfc
                this.renderer.image(`${window.location.href.includes('unctad.org') ? 'https://storage.unctad.org/2023-covid19_recovery/' : './'}assets/img/unctad_logo.svg`, 5, 15, 100, 100).add();
              }
            },
          },
          subtitle: {
      ...
      plotOptions: {
        column: {
          ...
          dataLabels: {
            ...
            formatter() {
              // eslint-disable-next-line react/no-this-in-sfc
              return (this.y === 0) ? '' : `${roundNr(this.y, data_decimals)}`;
            },
            color: 'rgba(0, 0, 0, 0.8)',
            style: {
              fontFamily: 'Roboto',
      ...
      xAxis: [{
        ...
        labels: {
          ...
          formatter: (val) => {
            if (val.value === 'empty1' || val.value === 'empty2') {
              return '';
            }
